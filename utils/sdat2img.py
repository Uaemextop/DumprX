...
def main(TRANSFER_LIST_FILE, NEW_DATA_FILE, OUTPUT_IMAGE_FILE):
    ...
    if not os.path.isfile(TRANSFER_LIST_FILE):
        print('Error: transfer list file "{}" not found'.format(TRANSFER_LIST_FILE), file=sys.stderr)
        sys.exit(1)

    if not os.path.isfile(NEW_DATA_FILE):
        print('Error: new data file "{}" not found'.format(NEW_DATA_FILE), file=sys.stderr)
        sys.exit(1)
    ...
    with open(NEW_DATA_FILE, 'rb') as new_data_file:
